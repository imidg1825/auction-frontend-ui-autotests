...
**RUN UI TESTS** — переход к workflow **UI Tests** в GitHub Actions для ручного запуска UI-тестов на CI с выбором набора `suite`.

Доступные варианты:

- `all` — полный прогон всех UI-тестов;
- `favorites_rtk_mutations` — отдельный прогон блока избранного под RTK Query Mutations.

1. Нажмите **Run workflow**.
2. В поле ветки выберите **`master`**.
3. В поле **suite** выберите `all` или `favorites_rtk_mutations`.
4. Подтвердите запуск (**Run workflow**).

⏱ **Время прогона:**

- `all` — ориентировочно **15–18 минут**;
- `favorites_rtk_mutations` — ориентировочно **3–5 минут**.

**Фреймворк для автоматизированного UI-тестирования аукционной платформы с полным циклом: авторизация, выполнение тестов, сбор результатов и публикация отчётов.**

https://front.test.kp.ktsf.ru/

На текущий момент в проекте реализовано **118 UI автотестов**.

📊 Последний прогон: 109 passed / 6 skipped
...
Проект покрывает ключевые пользовательские сценарии веб-приложения и построен как масштабируемый UI automation framework.

Фреймворк позволяет выполнять UI-тестирование независимо от состояния бэкенда за счёт прямой работы с фронтендом и автоматической авторизации.
...
Проект реализован как масштабируемый и переиспользуемый UI automation framework, предназначенный для стабильного тестирования фронтенда независимо от состояния бэкенда.

Основные принципы:

- разделение тестов по функциональным областям (auth, search, profile, auction и т.д.)
- использование общих фикстур (pytest + conftest.py)
- изоляция авторизации через сохранённое состояние (state.json)
- автоматическая авторизация через OTP (IMAP + Playwright)
- переиспользуемые утилиты (utils/)
...
- быстро добавлять новые тесты
- переиспользовать существующую логику
- масштабировать покрытие без усложнения кода
- запускать тесты локально и в CI одинаково
...
tests/
  ...
  test_auction.py
  test_favorites.py
  test_favorites_rtk_mutations.py
  test_home.py
...
При обновлении сессии используется автоматическая авторизация: OTP-код читается из почты и вводится в UI при запуске `python auth/save_auth.py` на хосте (контейнер для этого не нужен).

Рекомендуемый сценарий:
выполнить `python auth/save_auth.py` локально и смонтировать полученный `state.json` в контейнер — ручной ввод email и OTP внутри контейнера не требуется.
...
Далее откроется браузер:

- получение OTP-кода из почты
- автоматический ввод кода в форму
- сохранение `auth/state.json`
...
- автоматическая авторизация через OTP (получение кода из почты через IMAP)
